...
<html>
	...
	<body>
		...
		<div id="available-queries-intro" class="content" style="margin: 20px">
			Top 100 results for "${query.getTitle()}" ordered by their query likelihood score.
		</div>
		<div id="list-results" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<th>
						<g:message code="reldocs.label" default="Top 100 documents" />
					</th>
				</thead>
				<tbody>
				<g:each in="${resultsList}" status="i" var="result">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">	
						<td>
							...
							(score = ${fieldValue(bean: result, field: "score")})<br/>
